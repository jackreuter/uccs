...
public class BasicLookup {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println(getAllSegs("youaintgotnoyeezy"));

            while (line != null) {
                //process lines
                ArrayList<String> segs = getAllSegs(line);
                
            //     String capSegmentation = concatSingletons(segmentByCaps(line));
            //     String[] split = capSegmentation.split(" ");
            //     String segmentation = "";

            //     for (int i=0; i<split.length; i++) {
            //         String forwards = concatSingletons(ASM(1,1,1,split[i]));
            //         String backwards = concatSingletons(ASM(-1,1,1,split[i]));
            //         String merge = merge(forwards,backwards);
            //         segmentation += merge + " ";
            //     }
    ...
    public static ArrayList<String> getAllSegs(String s) {

        ArrayList<String> segs = new ArrayList<String>();
        if (s.length()==0) {return segs;}
        else {
            segs.add(s);
            ...
            for (int i=0; i<len && unknown; i++) {
                ...
                boolean frontIs = hasMatch(front);
                boolean backIs = hasMatch(back);

                if (frontIs) {
                    ArrayList<String> backSegs = getAllSegs(back);
                    for (int j=0; j<backSegs.size(); j++) {
                        String seg = front+" "+backSegs.get(j);
                        if (!segs.contains(seg)) {segs.add(seg);}
                    }
                }
                if (backIs) {
                    ArrayList<String> frontSegs = getAllSegs(front);
    ...
    public static boolean hasMatch(String s) {
        ...
        //else if (s.length()==1) {return true;}
            ...
            try {
                String file = "../dicts/total/try3/split/t"+len+".txt";
                BufferedReader br = new BufferedReader(new FileReader(file));
                String line = br.readLine();
                int count = 0;
                boolean done = false;
